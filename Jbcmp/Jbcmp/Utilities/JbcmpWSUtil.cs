...
namespace Hungsum.Jbcmp.Utilities
{
	public class JbcmpWSUtil : HsOAWSUtil
	{
		...
		public async Task<HsLabelValue> GetJbBjdxpsb(string progressId, string djId)
		{
			XElement xData = new XElement("Data",
								new XElement("ProgressId", progressId),
								new XElement("DjId", djId));

			string data = await postByName("GetJbBjdxpsb", HsGZip.CompressString(xData.ToString(SaveOptions.DisableFormatting)));
